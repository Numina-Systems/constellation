...
## Key Files
- `types.ts` -- `Agent`, `AgentConfig` (includes `max_skills_per_turn`, `skill_threshold`), `AgentDependencies` (includes optional `compactor`, `getExecutionContext`, `traceRecorder`, `embedding`, `owner`, `contextProviders`, `skills`, `sourceInstructions`, `beforeTurn`), `ConversationMessage`, `ExternalEvent`, `ContextProvider`
