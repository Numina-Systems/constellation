...
- **Guarantees**:
  ...
  - `execute_code` tool calls route to the Deno runtime; all other tools route through the registry
...
- **Expects**: All dependencies injected via `AgentDependencies`. Database connected with migrations applied.
...
## Key Files
- `types.ts` -- `Agent`, `AgentConfig`, `AgentDependencies`, `ConversationMessage`
