...
- **Guarantees**: All subsystem errors extend `ConstellationError` and carry `code` (string enum per subsystem), `subsystem` (identifier), `context` (serializable metadata), and optional `suggestion`. `toJSON()` safely serializes context (skipping unserializable values). `traceError()` records errors as operation traces fire-and-forget.
- **Expects**: Subsystem modules import their error class from here and throw it. Catch blocks use `traceError()` when a `TraceRecorder` is available.

## Dependencies
- **Uses**: `src/reflexion/` (optional, `TraceRecorder` type for `traceError`)
- **Used by**: All domain modules (`src/agent/`, `src/persistence/`, `src/shell/`, `src/model/`, `src/memory/`, `src/config/`)
