...
- **Used by**: All domain modules (`src/agent/`, `src/persistence/`, `src/shell/`, `src/model/`, `src/memory/`, `src/config/`, `src/compaction/`, `src/rate-limit/`, `src/secrets/`)
...
- `agent.ts`, `config.ts`, `memory.ts`, `model.ts`, `persistence.ts`, `shell.ts`, `secrets.ts` -- Subsystem error classes
