...
- **Used by**: `src/memory/postgres-store.ts`, `src/agent/agent.ts` (message persistence), `src/skill/postgres-store.ts` (skill embeddings), `src/reflexion/` (prediction store, trace recorder), `src/scheduler/` (scheduled tasks), `src/tool/builtin/scheduling.ts` (owner-scoped task queries), `src/index.ts`
