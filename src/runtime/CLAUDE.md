...
## Contracts
- **Exposes**: `CodeRuntime` interface (`execute(code, toolStubs, context?) -> ExecutionResult`), `ExecutionContext` type (includes optional `secrets` record), `generateCredentialConstants(context?)`, `generateSecretConstants(context?)`, `isValidIdentifier(key)`, `createDenoExecutor(config, registry)`, IPC message types
- **Guarantees**:
  ...
  - Deno permissions: granular allowlists by default (network to `allowed_hosts`, filesystem to `working_dir` + configured paths, subprocess to `allowed_run`, no env/FFI), or `--allow-all` when `unrestricted: true`
...
- **Note**: When `unrestricted: true`, the sandbox boundary is process isolation + resource limits only, not Deno permissions.
...
- Credential injection via constants: Optional `ExecutionContext` generates `const BSKY_*` declarations and agent-managed secret constants prepended to sandbox code, keeping secrets out of tool definitions
- Secret key validation: `isValidIdentifier()` prevents code injection by rejecting keys that aren't valid TypeScript identifiers; `generateSecretConstants()` silently skips invalid keys as defense-in-depth

## Invariants
- The Deno subprocess never has broader permissions than configured (unless `unrestricted: true`)
- All resource limits (size, time, call count) are enforced host-side, not trust-the-sandbox

## Key Files
- `types.ts` -- `CodeRuntime`, `ExecutionResult`, `ExecutionContext`, IPC message types
