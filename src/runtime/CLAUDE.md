...
## Contracts
- **Exposes**: `CodeRuntime` interface (`execute(code, toolStubs, context?) -> ExecutionResult`), `ExecutionContext` type, `generateCredentialConstants(context?)`, `createDenoExecutor(config, registry)`, IPC message types
- **Guarantees**:
...
- Credential injection via constants: Optional `ExecutionContext` generates `const BSKY_*` declarations prepended to sandbox code, keeping secrets out of tool definitions
